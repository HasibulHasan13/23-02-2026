...
export default function AdminLayout() {
    ...
    const navLinks = [
        ...
        { label: 'Manage VPS', path: '/admin/manage-vps' },
        { label: 'Manage Employees', path: '/admin/manage-employees' },
        { label: 'Marketing Notes', path: '/admin/marketing-notes' },
    ];
        ...
        <div className="admin-layout bg-background-light dark:bg-background-dark font-display text-gray-800 dark:text-gray-100 transition-colors duration-300 min-h-screen flex flex-col relative overflow-hidden">
            ...
            <nav className="w-full glass-panel sticky top-0 z-40 px-6 py-4 flex justify-between items-center border-b border-gray-200 dark:border-white/10">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="text-xl font-bold tracking-tight text-[#022c22] dark:text-white">
                        ARS <span className="text-admin-primary font-light">Surveys Admin</span>
                    </h1>
                </div>
                ...
                <div className="flex items-center gap-4">
                    {navLinks.map((link) => (
                        <Link
                            ...
                            className={`text-sm font-medium transition-colors ${
                                path === link.path
                                    ? 'text-admin-primary'
                                    : 'text-gray-500 dark:text-gray-300 hover:text-admin-primary'
                            }`}
                        >
                            {link.label}
                        </Link>
                    ...
                    <div className="flex items-center gap-3 pl-4 border-l border-gray-300 dark:border-white/10">
                        ...
                        <button onClick={logout} title="Logout" className="focus:outline-none">
                            <span className="material-symbols-outlined text-gray-600 dark:text-gray-300 hover:text-red-500 transition-colors cursor-pointer">
                                logout
                            </span>
                        </button>
                    </div>
